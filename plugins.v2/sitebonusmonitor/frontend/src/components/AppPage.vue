...
import { pluginBase, unwrapResponse, formatHourlyBonus, formatTb } from '../provider'
...
const topMetrics = computed(() => [...validMetrics.value].sort((a, b) => (b.hourly_bonus || 0) - (a.hourly_bonus || 0)))
...
defineExpose({ loadMetrics, loading })
</script>

<template>
  <div class="sitebonus-app-page">
    <div v-if="!hideTitle" class="d-flex align-center mb-4">
      <div class="text-h6">📊 站点魔力值监控</div>
      <VSpacer />
    ...
    <VAlert type="info" variant="tonal" density="compact" class="my-3">
      时魔 = 最近 24 小时内最早与最新魔力值快照的差值 / 小时数；窗口长度见「窗口(h)」列。
    </VAlert>
